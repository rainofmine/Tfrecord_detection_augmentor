...
def read_and_prepocess_single_img(filename_queue, shortside_len, is_training):
    ...
    if is_training:
        img = image_preprocess.color_aug(img)

        img, gtboxes_and_label = image_preprocess.random_crop(img_tensor=img, gtboxes_and_label=gtboxes_and_label)

        img, gtboxes_and_label = image_preprocess.random_size(img_tensor=img, gtboxes_and_label=gtboxes_and_label)

        img, gtboxes_and_label = image_preprocess.random_rotate(img_tensor=img, gtboxes_and_label=gtboxes_and_label)
...
def next_batch(batch_size, shortside_len, is_training):

    if is_training:
        pattern = os.path.join(cfg.ROOT_PATH, cfg.TFRECORD_PATH, cfg.DATASET_NAME + '_train*')
